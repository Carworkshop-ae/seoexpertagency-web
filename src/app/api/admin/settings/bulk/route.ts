...
import { getActingUser, SETTINGS_WRITE_ROLES } from '@/lib/auth-guard'
import { isSettingKey } from '@/types/settings'
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const unknownKey = parsed.data.settings.find(s => !isSettingKey(s.key))
    if (unknownKey) return NextResponse.json({ error: `Unknown setting key: ${unknownKey.key}` }, { status: 400 })
    ...
    await logAudit({ userId: acting.id, action: 'update', table: 'website_settings', recordId: parsed.data.settings.map(s => s.key).join(',') })
