...
interface Column<T> {
  ...
  className?: string
  /** Marks the column sortable; the parent sorts using `sortKey`/`sortDir`. */
  sortable?: boolean
}
...
interface DataTableProps<T extends Record<string, unknown>> {
  ...
  totalResults?: number
  /** Column key currently sorted, paired with `onSortChange` to enable sorting. */
  sortKey?: string
  sortDir?: SortDir
          ...
          <thead>
            <tr style={{ backgroundColor: '#1F2937' }}>
              ...
              {columns.map(col => {
                ...
                const canSort = !!onSortChange && !!col.sortable
                const active = canSort && sortKey === key
                return (
                  <th
                    key={key}
                    className={`px-4 py-3 text-left font-semibold text-white ${col.className ?? ''}`}
                  ...
                  >
                    {canSort ? (
                      <button
                        ...
                        className="inline-flex items-center gap-1 font-semibold text-white hover:text-[#93B4E8]"
                        aria-label={`Sort by ${col.header}`}
                      >
                        {col.header}
                        <span aria-hidden className={active ? 'text-white' : 'text-zinc-500'}>
                          {active ? (sortDir === 'asc' ? '▲' : '▼') : '↕'}
                        </span>
                      </button>
                    ) : col.header}
