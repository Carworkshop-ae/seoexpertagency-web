...
export function AddServiceCard() {
  ...
  function submit() {
    ...
    cards.create('services', { name: name.trim(), short_description: description.trim() || null, icon, status: 'published' }, `Add ${name.trim()}`)
    setName(''); setDescription(''); setIcon(SERVICE_ICONS[0].value); setAdding(false)
  }

  const wrap = (node: React.ReactNode) => (
    <>
      {cards.pendingCreates('services').map(c => {
        const PIcon = SERVICE_ICON_MAP[String(c.body.icon)] || DEFAULT_SERVICE_ICON
        return (
