  ...
  return (
    <section className="py-10" aria-labelledby="pricing-heading">
      <div className="max-w-2xl mx-auto px-4 sm:px-6">
        <h2 id="pricing-heading" className="text-2xl font-extrabold text-dark mb-5">{title}</h2>
        <div className="overflow-hidden rounded-2xl border border-hairline shadow-[var(--shadow-card)] bg-white">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-light-bg border-b border-hairline">
            ...
            <tbody className="divide-y divide-hairline">
              {items.map(item => (
                <tr key={item.label} className="hover:bg-light-bg transition-colors">
