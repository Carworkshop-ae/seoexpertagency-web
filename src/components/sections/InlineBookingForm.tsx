...
export function InlineBookingForm({ sourcePageSlug, prefillMessage, heading = 'Get a Free Quote' }: InlineBookingFormProps) {
  ...
  const inputCls = 'w-full px-3.5 py-3 rounded-xl border border-hairline bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all'

  return (
    <form onSubmit={submit} className="card-premium p-7 rounded-2xl">
      <h2 className="text-xl font-extrabold text-dark mb-1">{heading}</h2>
